...
const TASK_RECIPES = {
  "git-force-push-recovery": {
    ...
    timeEstimate: "3-5 hours for a senior software engineer, release engineer, or DevOps/SRE practitioner with hands-on Git internals experience.",
...
function buildFromRecipe(recipeId) {
  ...
  els.taskTime.value      = recipe.timeEstimate || timeEstimateFor(recipe.expertise, profile.domain);
...
async function buildAndDownloadZip() {
  ...
  if (lastTemplateState.domainKey === "git-workflows") {
    alert("Git recovery packages must be built by the local Runner so the hardened task-kit and golden-kit split, recovery cases, and pre-ship verifier gate are applied.");
    return;
  }
