...
if __name__ == "__main__":
  ...
  "typescript": {
    ...
    composePrompt(profile, type, standard, scenario) {
      const openers = {
        "post-migration validation": "After a type utility upgrade, AwaitedLike<T> silently widens Promise<never> union members to unknown. Fix it and produce outputs/fix.patch, outputs/tsc_report.json, outputs/type_test_results.json, and outputs/run_manifest.json.",
        "regression triage": "The TypeScript 5.x upgrade broke AwaitedLike<T> — Promise<never> members widen to unknown instead of resolving correctly. Fix it and produce outputs/fix.patch, outputs/tsc_report.json, outputs/type_test_results.json, and outputs/run_manifest.json.",
        "compliance audit": "A pre-release audit found AwaitedLike<T> widening Promise<never> to unknown under strict mode. Fix it without changing public type signatures. Produce outputs/fix.patch, outputs/tsc_report.json, outputs/type_test_results.json, and outputs/run_manifest.json.",
        "edge-case benchmark": "AwaitedLike<T> fails on an edge case: Promise<never> members infer unknown. Fix it without widening branches or changing public signatures. Produce outputs/fix.patch, outputs/tsc_report.json, outputs/type_test_results.json, and outputs/run_manifest.json."
      };
      const opener = openers[scenario && scenario.name] || openers["edge-case benchmark"];
      return [
        opener,
        "Four positive fixtures (normal_union.ts, nested_promise.ts, never_branch.ts, edge_deeply_nested.ts) must compile clean under tsconfig.strict.json. invalid_non_thenable.ts must produce exactly one TS2345 under tsconfig.negative.json. No public type signature changes in contracts/public_types.md.",
        standard.prompt
      ].join("\n\n");
    },
    sources: [
...
if __name__ == "__main__":
  ...
  "react": {
    ...
    composePrompt(profile, type, standard, scenario) {
      const openers = {
        "post-migration validation": "After the React 18 migration, DataFetcher commits stale async results when props change rapidly or the component unmounts mid-fetch. Fix it and produce outputs/DataFetcher.fixed.tsx, outputs/test_results.json, outputs/render_count_report.json, and outputs/run_manifest.json.",
        "regression triage": "DataFetcher has a regression: stale async results overwrite the rendered value on rapid prop changes or unmount. Fix it using the provided Jest fixtures and produce outputs/DataFetcher.fixed.tsx, outputs/test_results.json, outputs/render_count_report.json, and outputs/run_manifest.json.",
        "compliance audit": "An audit confirmed DataFetcher doesn't clean up async side effects on unmount, causing stale values and warnings. Fix it without changing the exported API. Produce outputs/DataFetcher.fixed.tsx, outputs/test_results.json, outputs/render_count_report.json, and outputs/run_manifest.json.",
        "edge-case benchmark": "DataFetcher has a stale-closure bug: async results overwrite state after unmount or rapid prop changes. Fix it so all five fixtures pass. Produce outputs/DataFetcher.fixed.tsx, outputs/test_results.json, outputs/render_count_report.json, and outputs/run_manifest.json."
      };
      const opener = openers[scenario && scenario.name] || openers["regression triage"];
      return [
        opener,
        "All 5 Jest fixtures must pass. Rapid-update: final rendered value equals the last dispatched request. Unmount fixture: zero 'state update on unmounted component' warnings. Render counts within expected_render_counts.json. No API changes in contracts/component_api.md.",
        standard.prompt
      ].join("\n\n");
    },
    sources: [
...
if __name__ == "__main__":
  ...
  "git-workflows": {
    ...
    composePrompt(profile, type, standard, scenario) {
      const openers = {
        "post-migration validation": "An accidental git push --force removed 3 commits from the release branch. Recover them with the exact topology specified in commit_graph_spec.json.",
        "regression triage": "Three commits went missing after an accidental force-push. Recover them with the correct parent chain using the provided bundles and reflog.",
        "compliance audit": "Incident review shows 3 commits are unreachable after an accidental force-push. Recover them with machine-verifiable checksum and topology evidence.",
        "edge-case benchmark": "The repo lost 3 commits from an accidental force-push. Recover them with proper reachability, parent-chain, and checksum verification — no cherry-picking."
      };
      const opener = openers[scenario && scenario.name] || openers["post-migration validation"];
      return [
        opener,
        "Produce outputs/repaired_repo.bundle, outputs/repair_log.json, outputs/commit_graph_report.json, and outputs/run_manifest.json. git fsck --connectivity-only must report 0 missing objects. All 3 recovered commits must be reachable with the parent chain from commit_graph_spec.json. File checksums must match expected_file_checksums.json. Branch refs must point to the specified SHAs.",
        standard.prompt
      ].join("\n\n");
    },
    sources: [
