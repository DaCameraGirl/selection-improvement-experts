...
const APP_VERSION = "2026-05-22 no-solver-in-input-zip";
...
function buildTaskPackageLines(fields) {
  ...
  return [
    ...
    " The Outlier form has 3 pages. Each section below is labeled PAGE 1, PAGE 2, or PAGE 3 to match.",
    "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
    "",
    "════════════════════════════════════════════════════",
    ...
    "",
    "── PAGE 1 · Final answer / Expected solution outputs ──",
    ...
    "── PAGE 1 · Verifier description (initial) ─────────",
    packageText(fields.verifiers, "Describe deterministic checks that accept correct outputs and reject incorrect outputs."),
    "",
    "── PAGE 1 · Time estimate ──────────────────────────",
    packageText(fields.time, "Estimate how long a qualified professional would need."),
    "",
    "── PAGE 1 · Agent difficulty check (if form asks) ──",
    packageText(fields.agentCheck, "Summarize terminal-agent testing only if you performed it."),
    "",
    "── PAGE 1 · Author email ───────────────────────────",
    "Enter your email on the Outlier form. Not generated here.",
    "",
    "════════════════════════════════════════════════════",
    " PAGE 2 — Golden Solution Detail",
    "════════════════════════════════════════════════════",
    "",
    "── PAGE 2 · Golden Solution Steps Description ──────",
    "Outlier provides an auto-generated draft on this page. Edit it into a granular, step-by-step description",
    "a non-expert coder can translate into terminal commands. Reuse the Golden Solution from PAGE 1 as your base,",
    "expand each step with prose + commands, and add a short 'Common failure modes' list at the end.",
    "",
    "Form rule: 'Please do not use an LLM for this portion of the task' — make at least 2-3 edits in your own voice.",
    "",
    "── PAGE 2 · Solution Summary ───────────────────────",
    "One short paragraph (3-6 sentences) summarizing the approach and the key insight a coder needs to know",
    "(e.g. the trap that causes naive implementations to fail). Reuse content from PAGE 1's Golden Solution and",
    "Difficulty fields as your base.",
    "",
    "════════════════════════════════════════════════════",
    " PAGE 3 — Verifiers Refinement",
    "════════════════════════════════════════════════════",
    "",
    "── PAGE 3 · Final Verifiers list ───────────────────",
    "Consolidate the Verifier description from PAGE 1 into a numbered list. Each item names the file/check, the",
    "exact pass condition, and a failure code (MISSING_FILE, SCHEMA_INVALID, TEST_FAIL, THRESHOLD_FAIL,",
    "CONTRACT_DRIFT, NON_DETERMINISTIC_OUTPUT, STDERR_WARNING). Include one item per output file combining",
    "presence + parseable JSON + schema, plus a determinism re-run check, plus the exit-code contract.",
    "",
    "── PAGE 3 · Verifiers Explanation ──────────────────",
    "Natural-language paragraph describing what the verifiers do as a pipeline: presence checks first, then",
    "cloneability + fsck, then cross-checks against fixture files, then schema/value checks, then determinism,",
    "then exit-code contract. End with: 'every check is a deterministic comparison or a tool/CLI command'.",
    "",
    "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
...
if __name__ == "__main__":
  ...
  "git-workflows": {
    ...
    composePrompt(profile, type, standard, scenario) {
      ...
      const constraintsLine = pickPhrase([
        "Please don't cherry-pick — it changes the commit IDs. Next, ensure the repaired bundle clones successfully, passes git fsck --connectivity-only with 0 missing or corrupt objects, matches expected_refs.json exactly, and satisfies the parent-chain and checksum fixtures.",
        "Cherry-picking is off the table since it changes SHAs. Match expected_refs.json exactly and make sure there are no missing or corrupt objects so we keep confidence in the recovery. The repaired bundle should clone cleanly, git fsck --connectivity-only needs to report 0 missing or corrupt objects, and the parent-chain and checksum fixtures all need to pass.",
        "No cherry-picking — that changes the original commit IDs. Next, the repaired bundle has to clone successfully, pass git fsck --connectivity-only with 0 missing or corrupt objects, match expected_refs.json exactly, and satisfy the parent-chain and checksum fixtures."
      ]);
      return [opener, filesLine, constraintsLine].join("\n\n");
    },
    sources: [
    ...
    downloads: [
      "orphaned_object_store/.git/objects and repo_after_force.bundle — included in the zip; no external download needed.",
...
def run(object_store, after_bundle, reflog_path, spec_path, out_dir):
    out = Path(out_dir); out.mkdir(parents=True, exist_ok=True)
    work = out / "work_repo"
    ...
    # Load dangling loose objects into the recovery repo, then surface them with fsck.
    object_objects = Path(object_store) / ".git" / "objects"
    for item in object_objects.iterdir():
        dest = work / ".git" / "objects" / item.name
        if item.is_dir():
            shutil.copytree(item, dest, dirs_exist_ok=True)
        elif item.is_file():
            shutil.copy2(item, dest)
    git(["fsck", "--lost-found"], cwd=work, check=False)
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser()
    ap.add_argument("--objects", default="orphaned_object_store")
    ...
    ap.add_argument("--out", default="outputs")
    args = ap.parse_args()
    run(args.objects, args.after, args.reflog, args.spec, args.out)`
    ,
...
# Clone the after-bundle, then graft dangling loose objects into the recovery repo.
...
import shutil
object_objects = object_store / '.git' / 'objects'
...
for item in object_objects.iterdir():
    ...
    if item.is_dir():
        ...
        shutil.copy2(item, dest)
git(['fsck', '--lost-found'], cwd=str(recovery))
...
const DOMAIN_CODE = {
  ...
  "git-workflows": {
    ...
    coreTodo: [
      ...
      "Copy dangling loose objects from orphaned_object_store/.git/objects into work_repo/.git/objects, then run git fsck --lost-found to surface unreachable commits",
...
function validateZipReadiness() {
  ...
  const hasGitResources = /(orphaned_object_store|repo_after_force\.bundle|reflog_export\.txt|commit_graph_spec\.json|expected_file_checksums\.json)/i.test(f.resources || "");
...
function buildDataReadme(domainKey, details, profile) {
  ...
  return ["# data/ — Download Instructions", "", "**Placeholder stubs only. Replace each file with the real download before implementing the task workflow.**", "", "## Public sources", "", ...sources.map((s) => `- ${s}`), "", ...(downloads.length ? ["## Step-by-step downloads", "", ...downloads.map((d, i) => `**${i + 1}.** ${d}`), ""] : []), "## Required files", "", ...resources.map((r) => `- ${r}`), "", `## Exclusion rules\nRecords are excluded if they: ${profile.failure || "fail schema validation or violate domain constraints"}.`].join("\n");
}

function buildFixturesReadme(domainKey, profile) {
  return ["# verifier_inputs/ — Fixture Documentation", "", "## correct.json", "A correct-output fixture used by the deterministic grader.", "", "## incorrect.json", "A deliberately broken-output fixture that the grader must reject.", `Failure reason: ${profile.threshold || "metrics below declared thresholds."}`, "", "## expected_metrics.json", "Numeric thresholds used by the grading contract. Update to match final task spec.", "", "## Smoke-test workflow", "1. Generate the required files under outputs/.", "2. Check that the grader accepts the correct fixture and rejects the incorrect fixture.", "3. Degrade one output by deleting a required column or lowering a metric below threshold.", "4. Confirm the degraded output is rejected.", "5. Document the failing step in 'Error if wrong' before submitting."].join("\n");
}
...
function buildZipReadme(domainKey, profile, scenario, details, fields) {
  ...
  const resources = (details && details.resources) ? details.resources : [];
  const isGitPackage = domainKey === "git-workflows";
  const quickStart = isGitPackage
    ? [
        "## Quick start",
        "```",
        "# Implement a recovery workflow that reads the supplied fixtures",
        "# and writes the required files under outputs/.",
        "# Reference scripts are intentionally not included here.",
        "```"
      ]
    : [
        "## Quick start",
        ...
        "# Place real data files in data/ (see data/README_data.md)",
        "# Implement the task workflow and write the required outputs.",
        "```"
      ];
  const folderRows = isGitPackage
    ? [
    ...
    : [
        "| `config/` | Task-specific thresholds and parameters |",
        "| `data/` | Input data - see data/README_data.md |",
        "| `schemas/` | JSON Schema definitions for output validation |",
        "| `verifier_inputs/` | Correct and incorrect fixture files |",
        "| `adversarial_tests/` | Schema-break, boundary, and conflict test cases |",
        "| `huggingface_export/` | HuggingFace-style JSONL dataset export |",
        "| `outputs/` | Worker-created output artifacts |",
        "| `environment/` | requirements.txt and version manifest |"
      ];
  const verifierLines = isGitPackage
    ? [
        "## Grading behavior",
        "",
        "- Required outputs are checked for presence, schema, and consistency.",
        "- The repaired bundle must clone and pass `git fsck --connectivity-only`.",
        "- Restored refs must match `expected_refs.json` exactly."
      ]
    : [
        "## Verifier behavior",
        "",
        "- **Exit 0** - all required files present, schemas valid, metrics within tolerance",
        "- **Exit 1** - missing file, schema violation, metric below threshold, or invalid fixture accepted"
      ];
  return [
    `# Task Package: ${fields.title || profile.brief}`,
    "",
    `**Domain:** ${DOMAIN_CATEGORY[domainKey] || profile.domain}`,
    `**Scenario:** ${scenario.name}`,
    `**Generated:** ${new Date().toISOString().slice(0, 10)} via Selection Improvement Experts ${APP_VERSION}`,
    "",
    ...quickStart,
    "",
    "## Folder structure",
    ...
    "## Threshold contract",
    "",
    profile.threshold || "See verifier_inputs/expected_metrics.json for numeric thresholds.",
    "",
    "## Data sources",
    "",
    ...sources.map((s) => `- ${s}`),
    "",
    ...(downloads.length ? ["## Download steps", "", ...downloads.map((d) => `- ${d}`), ""] : []),
    ...
    ...verifierLines,
    "",
    "> The workflow must run without network access after unpacking."
  ].join("\n");
}
...
async function buildAndDownloadZip() {
  ...
  try {
    ...
    // Core worker-facing files. Do not include solve.py or verify.py in the
    // task package; those belong only in the golden/reference attachment.
    ...
    outFolder.file("README_outputs.md", "# outputs/\n\nWorker-created output artifacts belong here. The worker-facing task package intentionally does not include reference scripts.\n");
...
let lastBuiltZipInfo = null;

function getRunnerDownloadInfo(dataOrTaskId, family = "task") {
  const taskId = typeof dataOrTaskId === "string" ? dataOrTaskId : dataOrTaskId?.task_id;
  const base = RUNNER_API_BASE;
  if (!taskId) return null;
  const data = typeof dataOrTaskId === "object" && dataOrTaskId ? dataOrTaskId : {};
  return {
    taskId,
    family: data.family || family || "task",
    legacyUrl: data.download_url ? `${base}${data.download_url}` : `${base}/api/download/${taskId}`,
    taskKitUrl: data.task_kit?.download_url ? `${base}${data.task_kit.download_url}` : `${base}/api/download/${taskId}_task_kit`,
    goldenKitUrl: data.golden_kit?.download_url ? `${base}${data.golden_kit.download_url}` : `${base}/api/download/${taskId}_golden_kit`
  };
}

function updateRunnerDownloadLinks(info) {
  if (!info) return;
  const legacyLink = document.querySelector("#runner-download-zip");
  const taskLink = document.querySelector("#runner-download-task-kit");
  const goldenLink = document.querySelector("#runner-download-golden-kit");
  if (legacyLink) {
    legacyLink.href = info.legacyUrl;
    legacyLink.download = `${info.family}-all-in-one-${info.taskId.slice(0, 8)}.zip`;
    legacyLink.classList.remove("is-hidden");
  }
  if (taskLink) {
    taskLink.href = info.taskKitUrl;
    taskLink.download = `${info.family}-task-kit-${info.taskId.slice(0, 8)}.zip`;
    taskLink.textContent = "Download Task Kit";
    taskLink.classList.remove("is-hidden");
  }
  if (goldenLink) {
    goldenLink.href = info.goldenKitUrl;
    goldenLink.download = `${info.family}-golden-kit-${info.taskId.slice(0, 8)}.zip`;
    goldenLink.textContent = "Download Golden Kit";
    goldenLink.classList.remove("is-hidden");
  }
}
...
function renderSplitZipLinks(info, uploaded = false) {
  if (!info) return "";
  return `<span class="runner-check">✓</span> Package built: <strong>${escapeHtml(info.family)}</strong> (${escapeHtml(info.taskId)})${uploaded ? " — auto-uploaded to Runner" : ""}` +
    `<div class="runner-actions" style="margin-top:.5rem">` +
    `<a class="button-link" href="${escapeHtml(info.taskKitUrl)}" download>Download Task Kit</a>` +
    `<a class="button-link" href="${escapeHtml(info.goldenKitUrl)}" download>Download Golden Kit</a>` +
    `</div>`;
}
...
async function handleBuildTaskZip() {
  ...
  const statusEl = document.querySelector("#runner-pipeline-status") || document.querySelector("#runner-builder-status");
...
try {
    ...
    lastBuiltZipInfo = getRunnerDownloadInfo(data, data.family);
    updateRunnerDownloadLinks(lastBuiltZipInfo);
    ...
    if (statusEl) statusEl.innerHTML = renderSplitZipLinks(lastBuiltZipInfo, uploaded);
    if (downloadLink) {
      downloadLink.href = lastBuiltZipInfo.taskKitUrl;
...
async function downloadRunnerTaskZip() {
  ...
  if (statusEl) statusEl.innerHTML = lastBuiltZipId ? "Downloading latest split ZIPs..." : "Building runner ZIP package...";
  ...
  try {
    ...
    if (lastBuiltZipId) {
      const info = lastBuiltZipInfo || getRunnerDownloadInfo(lastBuiltZipId, spec.family);
      lastBuiltZipInfo = info;
      updateRunnerDownloadLinks(info);
      const downloadLink = document.querySelector("#runner-download-zip");
      if (downloadLink) {
        downloadLink.href = info.taskKitUrl;
        downloadLink.classList.remove("is-hidden");
      }

      clickDownload(info.taskKitUrl, `${spec.family}-task-kit-${lastBuiltZipId.slice(0, 8)}.zip`);
      setTimeout(() => clickDownload(info.goldenKitUrl, `${spec.family}-golden-kit-${lastBuiltZipId.slice(0, 8)}.zip`), 300);

      if (statusEl) statusEl.innerHTML = `<span class="runner-check">✓</span> Downloaded Task Kit and Golden Kit: <strong>${spec.family}</strong> (${lastBuiltZipId})`;
      return;
    }
    ...
    clickDownload(lastBuiltZipInfo.taskKitUrl, `${data.family}-task-kit-${data.task_id.slice(0, 8)}.zip`);
    setTimeout(() => clickDownload(lastBuiltZipInfo.goldenKitUrl, `${data.family}-golden-kit-${data.task_id.slice(0, 8)}.zip`), 300);

    // Split downloads are triggered above; keep the legacy all-in-one zip hidden.
    if (statusEl) statusEl.innerHTML = renderSplitZipLinks(lastBuiltZipInfo, false);
    setTimeout(() => { if (statusEl) statusEl.innerHTML = renderSplitZipLinks(lastBuiltZipInfo, false); }, 0);
...
function buildComputedTaskPackage(domainKey, fields) {
  ...
  if (faIdx === -1) {
    const modernIdx = parts.findIndex((line) =>
      line.includes("Final answer / Expected solution outputs") ||
      (line.includes("Final answer") && line.includes("Expected solution outputs"))
    );
    const isSectionHeader = (line) =>
      line.startsWith("── PAGE ") ||
      line.startsWith("── FIELD:") ||
      line.startsWith("â”€â”€ PAGE ") ||
      line.startsWith("â”€â”€ FIELD:");
    if (modernIdx !== -1) {
      const nextField = parts.slice(modernIdx + 1).findIndex(isSectionHeader);
      const endIdx = nextField !== -1 ? modernIdx + 1 + nextField : parts.length;
      parts.splice(modernIdx, endIdx - modernIdx, parts[modernIdx], faText, "");
    } else {
      parts.push("", "── PAGE 1 · Final answer / Expected solution outputs ──", faText, "");
    }
  }
  return parts.join("\n");
}
...
async function handleImportComputedOutputs() {
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      if (statusEl) statusEl.innerHTML = `<span class="runner-cross">✗</span> Import failed: ${escapeHtml(err.placeholder_reason || err.error || `HTTP ${resp.status}`)}`;
      return;
    }
    const data = await resp.json();
    if (data.status === "COMPUTED_PASS") {
      ...
      statusEl.innerHTML = `<span class="runner-cross">✗</span> Import finished, but finalize returned ${escapeHtml(data.status || "an unknown status")}.`;
    }
  } catch (err) {
...
async function startRun(runSetup, runSolve, runVerify) {
  ...
  try {
    ...
    const openLogsBtn = document.querySelector("#runner-open-logs");
    const downloadLogsBtn = document.querySelector("#runner-download-logs");
    if (openLogsBtn) openLogsBtn.disabled = false;
    if (downloadLogsBtn) downloadLogsBtn.disabled = false;
  ...
  const poll = async () => {
    try {
      ...
      if (terminalStates.includes(data.status)) {
        ...
        if (openLogsBtn) openLogsBtn.disabled = false;
        if (downloadLogsBtn) downloadLogsBtn.disabled = false;
        // Enable import outputs button once outputs exist; finalize happens inside import.
        if (data.status === "OUTPUTS_COLLECTED" || data.status === "COMPUTED_PASS") {
          document.querySelector("#runner-import-outputs").disabled = false;
        }
      }
    } catch {
...
async function runEnterprisePipeline() {
  ...
  try {
    ...
    lastBuiltZipInfo = getRunnerDownloadInfo(buildData, buildData.family || family);
    updateRunnerDownloadLinks(lastBuiltZipInfo);
    const downloadLink = document.querySelector("#runner-download-zip");
    if (downloadLink) {
      downloadLink.href = lastBuiltZipInfo.taskKitUrl;
